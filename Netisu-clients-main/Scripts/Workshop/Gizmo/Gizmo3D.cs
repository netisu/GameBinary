...
public partial class Gizmo3D : Node3D
{
	...
	public static Gizmo3D Instance { get; private set; } = null!;
	...
	public ToolMode Mode { get; set; } = ToolMode.Move;
	...
	public uint Layers
	{
		...
		set
		{
			...
			{
				InstanceSetLayerMask(Selection.Value.Item.SboxInstance, Layers);
				InstanceSetLayerMask(Selection.Value.Item.SboxInstanceOffset, Layers);
				InstanceSetLayerMask(Selection.Value.Item.SboxXrayInstance, Layers);
				InstanceSetLayerMask(Selection.Value.Item.SboxXrayInstanceOffset, Layers);
			}
		}
	}
	...
	/// </summary>
	public SelectedInstance? Selection = null;
	/// <summary>
	...
	[Flags]
	public enum ToolMode { Select = 0, Move = 1, Rotate = 2, Scale = 4, All = 7 };
	...
	public struct SelectedInstance
	{
		public ActiveInstance Instance;
	...
	{
		// If a playtest is running, the gizmo should not process any input.
		if (Engine3D.Instance != null && Engine3D.Instance.PlayTest)
		{
			return;
		}
		Hovering = false;
	...
	public void Select(ActiveInstance target)
	{
		//Selection[target] = GetEditorData();
		Selection = new()
		{
			Instance = target,
			Item = GetEditorData()
	...
	public bool Deselect(Node3D target)
	{
		...
		SelectedItem selectedItem = Selection.Value.Item;

		Selection = null;
		FreeRid(selectedItem.SboxInstance);
		FreeRid(selectedItem.SboxInstanceOffset);
		FreeRid(selectedItem.SboxXrayInstance);
		FreeRid(selectedItem.SboxXrayInstanceOffset);
		return true;
	}
	...
	{
		// We compare the 'Container' property of the selected instance, which is the actual Node3D in the scene.
		if (!Selection.HasValue || Selection.Value.Instance == null)
			return false;

		return Selection.Value.Instance.Container == target;
	}
	...
	public void ClearSelection()
	{
		if (Selection == null)
			return;
		FreeRid(Selection.Value.Item.SboxInstance);
		FreeRid(Selection.Value.Item.SboxInstanceOffset);
		FreeRid(Selection.Value.Item.SboxXrayInstance);
		FreeRid(Selection.Value.Item.SboxXrayInstanceOffset);
...
void fragment() {
	...
	void UpdateTransformGizmoView()
	{
		...
		if (!Selection.HasValue || !IsInstanceValid(Selection.Value.Instance) || !IsInstanceValid(Selection.Value.Instance.Container))
		{
			return;
		}

		var item = Selection.Value;
		var containerNode = item.Instance.Container;

		if (containerNode == null) return;
		Aabb bounds = CalculateSpatialBounds(item.Instance.Container);

		Vector3 offset = new(0.005f, 0.005f, 0.005f);
		Basis aabbS = Basis.FromScale(bounds.Size + offset);
		Transform3D t = item.Instance.Container.GlobalTransform.TranslatedLocal(bounds.Position - offset / 2);
		t.Basis *= aabbS;

		offset = new(0.01f, 0.01f, 0.01f);
		aabbS = Basis.FromScale(bounds.Size + offset);
		Transform3D tOffset = item.Instance.Container.GlobalTransform.TranslatedLocal(bounds.Position - offset / 2);
		tOffset.Basis *= aabbS;

		InstanceSetTransform(item.Item.SboxInstance, t);
		InstanceSetVisible(item.Item.SboxInstance, ShowSelectionBox);
		InstanceSetTransform(item.Item.SboxInstanceOffset, tOffset);
		InstanceSetVisible(item.Item.SboxInstanceOffset, ShowSelectionBox);
		InstanceSetTransform(item.Item.SboxXrayInstance, t);
		InstanceSetVisible(item.Item.SboxXrayInstance, ShowSelectionBox);
		InstanceSetTransform(item.Item.SboxXrayInstanceOffset, tOffset);
		InstanceSetVisible(item.Item.SboxXrayInstanceOffset, ShowSelectionBox);
	...
	void SetVisibility(bool visible)
	{
		...
		var item = Selection.Value.Item;
		InstanceSetVisible(item.SboxInstance, visible);
		InstanceSetVisible(item.SboxInstanceOffset, visible);
		InstanceSetVisible(item.SboxXrayInstance, visible);
		InstanceSetVisible(item.SboxXrayInstanceOffset, visible);
	...
	{
		// This prevents crashes if the object was freed since the last frame.
		if (!Selection.HasValue || !GodotObject.IsInstanceValid(Selection.Value.Instance) || !GodotObject.IsInstanceValid(Selection.Value.Instance.Container))
		{
			if (Visible)
			{
				SetVisibility(false);
			...
			// Also clear the invalid selection to prevent repeated errors.
			Selection = null;
			return;
		}
		...
		Transform3D xf = Selection.Value.Instance.Container.GlobalTransform;
	...
	bool TransformGizmoSelect(Vector2 screenpos, bool highlightOnly = false)
	{
		...
		if (!Visible || !Selection.HasValue)
		{
			if (highlightOnly) SelectGizmoHighlightAxis(-1);
			return false;
		}

		Vector3 rayPos = GetRayPos(camera, screenpos);
		Vector3 ray = GetRay(camera, screenpos);
		...
		if ((Mode & ToolMode.Rotate) == ToolMode.Rotate)
		{
			...
			if (result.Length != 0)
			{
				...
				if (hitNormal.Dot(GetCameraNormal(camera)) < 0.05f)
				{
			...
			if (colAxis == -1)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					if (GetCameraNormal(camera).Dot(rDir) <= 0.005f)
					{
	...
	{
		// Get the camera once and check for null before proceeding.
		Camera3D camera = GetViewport().GetCamera3D();
		if (camera == null)
		{
			return default;
		}
		Vector3 rayPos = GetRayPos(camera, Edit.MousePos);
		Vector3 ray = GetRay(camera, Edit.MousePos);
		...
		switch (Edit.Mode)
		{
			case TransformMode.Scale:
				...
				switch (Edit.Plane)
				{
					case TransformPlane.View:
						...
						splane = new(GetCameraNormal(camera), Edit.Center);
						break;
					case TransformPlane.X:
						smotionMask = Transform.Basis[0].Normalized();
						splane = new(smotionMask.Cross(smotionMask.Cross(GetCameraNormal(camera))).Normalized(), Edit.Center);
						break;
			...
			case TransformMode.Translate:
				...
				{
					case TransformPlane.View:
						tplane = new(GetCameraNormal(camera), Edit.Center);
						break;
					case TransformPlane.X:
						tmotionMask = Transform.Basis[0].Normalized();
						tplane = new(tmotionMask.Cross(tmotionMask.Cross(GetCameraNormal(camera))).Normalized(), Edit.Center);
						break;
					case TransformPlane.Y:
			...
			case TransformMode.Rotate:
				...
				if (camera.Projection == Camera3D.ProjectionType.Perspective)
				{
					...
					if (!camToObj.IsZeroApprox())
						...
						rplane = new(GetCameraNormal(camera), Edit.Center);
				}
				else
				{
					rplane = new(GetCameraNormal(camera), Edit.Center);
	...
	void ApplyTransform(Vector3 motion, float snap)
	{
		...
		var item = Selection.Value;
		Transform3D newTransform = ComputeTransform(Edit.Mode, item.Item.TargetGlobal, item.Item.TargetOriginal, motion, snap, localCoords, Edit.Plane != TransformPlane.View);
		TransformGizmoApply(item.Instance.Container, newTransform, localCoords);
		UpdateTransformGizmo();
	...
	void ComputeEdit(Vector2 point)
	{
		...
		Edit.ClickRay = GetRay(camera, point);
		Edit.ClickRayPos = GetRayPos(camera, point);
		Edit.Plane = TransformPlane.View;
		UpdateTransformGizmo();
		Edit.Center = Transform.Origin;
		Edit.Original = Transform;

		SelectedItem item = Selection.Value.Item;
		item.TargetGlobal = Selection.Value.Instance.Container.GlobalTransform;
		item.TargetOriginal = Selection.Value.Instance.Container.Transform;
		Selection = new()
		{
			Instance = Selection.Value.Instance,
			Item = item
		};
	}

	static Aabb CalculateSpatialBounds(Node3D parent, bool omitTopLevel = false, Transform3D boundsOrientation = default)
	{
		Aabb bounds;
	...
	Vector3 GetRayPos(Camera3D camera, Vector2 pos) => camera.ProjectRayOrigin(pos);
	Vector3 GetRay(Camera3D camera, Vector2 pos) => camera.ProjectRayNormal(pos);
	Vector3 GetCameraNormal(Camera3D camera) => -camera.GlobalTransform.Basis[2];
